...
exports.generateTennisScore = exports.UCL_MATCHES = exports.ALL_TENNIS_MATCHES = exports.TENNIS_MATCHES = exports.ALL_FOOTBALL_MATCHES = exports.getGroupMatches = exports.TOP_SCORERS = exports.TEAMS = void 0;
...
var getMatchScheduleInfo = function (group, matchIdx) {
    var key = "".concat(group, "-").concat(matchIdx);
...
var getGroupMatches = function (group, teams) {
    var groupTeams = teams.filter(function (t) { return t.group === group; });
    ...
    return basicMatches.map(function (_a) {
        var idx = _a.idx, home = _a.home, away = _a.away;
        var id = "".concat(group, "-").concat(idx);
        var info = getMatchScheduleInfo(group, idx);
        return __assign({ id: id, home: home, away: away, group: group }, info);
    });
};
exports.getGroupMatches = getGroupMatches;
exports.ALL_FOOTBALL_MATCHES = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L'].flatMap(function (group) { return (0, exports.getGroupMatches)(group, exports.TEAMS); });
// --- Tennis Sandbox Data ---
exports.TENNIS_MATCHES = [
    {
...
exports.ALL_TENNIS_MATCHES = exports.TENNIS_MATCHES.sort(function (a, b) { return a.timestamp - b.timestamp; });
// --- UCL Sandbox Data ---
exports.UCL_MATCHES = [
    {
        id: 'ucl_2026_final',
...
// --- Helpers ---
var generateTennisScore = function (bestOf) {
    var isHomeWinner = Math.random() > 0.5;
    if (bestOf === 3) {
        var loserSets = Math.random() > 0.5 ? 1 : 0;
        return {
        ...
        var loserSets = rand > 0.66 ? 2 : rand > 0.33 ? 1 : 0;
        return {
...
exports.generateTennisScore = generateTennisScore;
function calculateMatchPoints(sport, predictedHome, predictedAway, actualHome, actualAway, predictedProps, actualProps) {
    var points = 0;
    if (sport === 'ucl') {
        // UCL Specific Rules
        if (predictedHome !== '' && predictedAway !== '' && actualHome !== '' && actualAway !== '') {
            var pHome = Number(predictedHome);
            var pAway = Number(predictedAway);
            var aHome = Number(actualHome);
            var aAway = Number(actualAway);
            if (pHome === aHome && pAway === aAway)
                points += 10;
            else {
                var predictedWinner = pHome > pAway ? 'home' : pHome < pAway ? 'away' : 'draw';
                var actualWinner = aHome > aAway ? 'home' : aHome < aAway ? 'away' : 'draw';
        ...
        if (typeof predictedProps === 'object' && typeof actualProps === 'object') {
            var pp = predictedProps;
            var ap = actualProps;
            if (pp['top_scorer'] && pp['top_scorer'] === ap['top_scorer'])
                points += 3;
            if (pp['first_goalscorer'] && pp['first_goalscorer'] === ap['first_goalscorer'])
                points += 3;
            if (pp['last_goalscorer'] && pp['last_goalscorer'] === ap['last_goalscorer'])
                points += 3;
            if (pp['total_corners'] && pp['total_corners'] === ap['total_corners'])
                points += 1;
            if (pp['yellow_cards'] && pp['yellow_cards'] === ap['yellow_cards'])
                points += 1;
            if (pp['red_cards'] && pp['red_cards'] === ap['red_cards'])
                points += 1;
        }
        // Single Prop Bet backwards compatibility
        if (typeof predictedProps === 'string' && typeof actualProps === 'string') {
            if (predictedProps === actualProps)
                points += 3;
        }
        return points;
    }
        ...
        var predictedWinner = pHome > pAway ? 'home' : 'away';
        var actualWinner = aHome > aAway ? 'home' : 'away';
        if (predictedWinner === actualWinner)
            return 1;
        return 0;
    }
...
function getMatchStatus(match, now) {
    ...
    var duration = match.sport === 'tennis' ? 4 * 60 * 60 * 1000 : 2.5 * 60 * 60 * 1000;
...
function calculateTotalScore(predictionsData, targetSport, liveResults) {
    if (!predictionsData)
        return 0;
    var total = 0;
    ...
    if (!targetSport || targetSport === 'ucl') {
        var uPreds_1 = predictionsData.uPreds || {};
        exports.UCL_MATCHES.forEach(function (match) {
            var matchData = live[match.id] || match;
            if (matchData.actualHomeScore !== undefined && matchData.actualHomeScore !== null && matchData.actualAwayScore !== undefined && matchData.actualAwayScore !== null) {
                var p = uPreds_1[match.id];
                if (p) {
                    total += calculateMatchPoints('ucl', p.homeScore, p.awayScore, matchData.actualHomeScore, matchData.actualAwayScore, p.propBets || p.selectedProp, matchData.actualPropBets || match.actualPropBets || {});
                }
            }
        });
    }
    ...
    if (!targetSport || targetSport === 'tennis') {
        var tPreds_1 = predictionsData.tPreds || {};
        exports.ALL_TENNIS_MATCHES.forEach(function (match) {
            var matchData = live[match.id] || match;
            if (matchData.actualHomeScore !== undefined && matchData.actualHomeScore !== null && matchData.actualAwayScore !== undefined && matchData.actualAwayScore !== null) {
                var p = tPreds_1[match.id];
                if (p) {
                    total += calculateMatchPoints('tennis', p.homeScore, p.awayScore, matchData.actualHomeScore, matchData.actualAwayScore);
                }
            }
        });
    }
    ...
    if (!targetSport || targetSport === 'football') {
        var fPreds_1 = predictionsData.fPreds || {};
        exports.ALL_FOOTBALL_MATCHES.forEach(function (match) {
            var matchData = live[match.id] || match;
            if (matchData.actualHomeScore !== undefined && matchData.actualHomeScore !== null && matchData.actualAwayScore !== undefined && matchData.actualAwayScore !== null) {
                var p = fPreds_1[match.id];
                if (p) {
                    total += calculateMatchPoints('football', p.homeScore, p.awayScore, matchData.actualHomeScore, matchData.actualAwayScore);
                }
            }
        });
    }
